...
export const validateUser = (req, res, next) => {
  if (EmailValidator.validate(req.body.user.email)) {
    ...
    res
      .status(400)
      .json({ error: "algun campo que has introducido no es válido" });
  }
};
/**
...
export const validateAuth = (req, res, next) => {
  try {
    ...
    const token = auth.split(" ")[1]; // obtenemos el token// La primera posicion es el Bearer. Queremos el token [1]
...
export function getEmailToken(req, res) {
  try {
    // obtener el email --> lo tengo que obtener del token
    const auth = req.header("Authorization"); // me devuelve el valor de la header
    // ¿Que estructura tiene la header? --> Bearer _token_jwt_
